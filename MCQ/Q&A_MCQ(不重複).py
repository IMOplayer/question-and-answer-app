...
def change():
    """選擇問題和ICON跳到答題介面"""
    global doc, win, ans_A, ans_B, ans_C, ans_D, pq, que, entry, button1, button2, doc_var, ico_var, temp ,time, times
    ...
    buttonD.pack()
    time = tk.Label(win, height=10, width=10, font=15, textvariable=times, bg="#FFFFFF")
    time.pack()
...
def get_question():
    ...
    for t in doc.paragraphs:
        ...
        for run in t.runs:
            ...
            if run.underline:
                flag = True
        if temp_text[0] == "A":  # 判斷取得文字是題目還是A,B,C,D
            A.append(temp_text)
        elif temp_text[0] == "B":
            B.append(temp_text)
        elif temp_text[0] == "C":
            ...
            question.append(temp_text)
            flag = False
        if flag:
            ans.append(temp_text[0])


def update():
    """更新問題"""
    global ans_A, ans_B, ans_C, ans_D, pq, num, temp, times
    if num is not None:
        temp.remove(num)  # 刪去選過的題
    if len(temp) == 0:  # 如果全部選完，更新剩餘的問題
        temp = [i for i in range(len(question))]
